...
let tabAppend = (f) => {
	let t = new Float32Array(tab_size);
	for (let i = 0; i < tab_size; i++) {
		t[i] = f(i);
	}
	tab.push(t);
...
tabAppend((i) => Math.sin(i * 6.283184 / tab_size));
tabAppend((i) => tab[0][i] < 0 ? -1 : 1);
tabAppend((i) => i / tab_size - 0.5);
tabAppend((i) => i < tab_size / 2 ? (i / (tab_size / 4)) - 1 : 3 - (i / (tab_size / 4)));
	...
	return [
		(out1, out2, samples) => {
			...
			while (samples > 0) {
				...
				processing = Math.min(bufferLen, events.length ? Math.min(samples, events[0][0] - time) : samples);

				if (LFO_WAVEFORM) {
					for (let k = 0; k < processing; ++k) {
						lfoBuffer[k] = LFO_WAVEFORM[(lfoPhase & tab_mask)] * LFO_AMT + 0.5;
						lfoPhase += LFO_FREQ;
					}
				}
		...
		(note, t = 0) => scheduleEvent(
			() => {
				...
				oscillators.push(() => {
					...
					let osc1_freq = Math.pow(1.059463094, (note + OSC1_OCT + OSC1_DET) - 128) * OSC1_DETUNE;
					let osc2_freq = Math.pow(1.059463094, (note + OSC2_OCT + OSC2_DET) - 128) * OSC2_DETUNE;
		...
		([cmd, v], t = 0) => scheduleEvent(
			() => ([
				...
				v => OSC1_XENV = v,
				v => OSC1_VOL = v,
				v => OSC1_WAVEFORM = tab[v],
				v => OSC2_OCT = (v - 8) * 12,
				v => OSC2_DET = v,
				v => OSC2_DETUNE = 0.00390625 * (1 + 0.0008 * v),
				v => OSC2_XENV = v,
				v => OSC2_VOL = v,
				v => OSC2_WAVEFORM = tab[v],
				v => NOISE_FADER = v * 4.6566e-010,
				v => ENV_ATTACK = v,
				v => ENV_SUSTAIN = v,
				v => ENV_RELEASE = v,
				v => ENV_MASTER = v,
				...
				v => FX_PAN_FREQ = Math.pow(2, v - 8) / rowLen,
				v => FX_PAN_AMT = v / 512,
				v => LFO_OSC1_FREQ = v,
				v => LFO_FX_FREQ = v,
				v => LFO_FREQ = (Math.pow(2, v - 8) / rowLen) * tab_size,
				v => LFO_AMT = v / 512,
				v => LFO_WAVEFORM = tab[v],
			][cmd](v || 0)),
			t
		)
	];
...
let pl_synth_init = (ctx) => {
	let sound = (instrument, note = 147 /* C-5 */, row_len = 5513 /* 120 BPM */) => {
		...
		const [render, playNote, cmd] = channel(row_len, num_samples, samplerate);
	...
	let song = (songData) => {
		...
		for (let track of tracks) {
			...
			const [render, playNote, cmd] = channel(row_len, 100_000, samplerate);
